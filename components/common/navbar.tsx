...
export function Navbar() {
  ...
  return (
    <nav className="sticky top-0 z-50 bg-primary/30 backdrop-blur-md  text-primary-foreground shadow-lg">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-16">
          ...
          <Link href="/" className="flex items-center gap-3 font-bold text-xl hover:opacity-80 transition-opacity">
            ...
            <span className="hidden sm:inline">IGSL</span>
          </Link>
          ...
          <div className="hidden md:flex items-center gap-1">
            {navLinks.map((link) => (
              <Link key={link.href} href={link.href}>
                <Button  className="text-primary-foreground hover:bg-white hover:text-green-600">
                  {link.label}
                </Button>
              </Link>
            ))}
